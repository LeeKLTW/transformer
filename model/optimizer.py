...
class LazyAdam(tf.keras.optimizers.Adam):
  ...
  def _resource_apply_sparse(self, grad, var, indices):
    ...
    m_update_op = tf.scatter_update(**m_update_kwargs)

    # todo what is this means?
    # \\(v := beta2 * v + (1 - beta2) * (g_t * g_t)\\)
    v = self.get_slot(var, 'v')
    v_t_slice = (beta_2_t * tf.gather(v, indices) +
                 (1 - beta_2_t) * tf.square(grad))

    v_update_kwargs = {
        'indices': indices,
        'updates': v_t_slice
    }
    v_update_op = tf.scatter_update(**v_update_kwargs)

    # \\(variable -= learning_rate * m_t / (epsilon_t + sqrt(v_t))\\)
    var_slice = lr * m_t_slice / (tf.sqrt(v_t_slice) + epsilon_t)

    var_update_kwargs = {
        'indices': indices,
        'updates': var_slice
    }
    var_update_op = tf.scatter_update(**var_update_kwargs)
